...
```console
git clone https://github.com/FabianSatoshow/forms.git ajax
```
...
Similar to other [constructor functions](http://atom-Sato.github.io/object-oriented-javascript/){:target="_blank"} such as `Date`, we create this object using the `new` keyword setting it to a variable that we can interact with. 
